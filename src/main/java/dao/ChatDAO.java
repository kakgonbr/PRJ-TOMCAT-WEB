...
public class ChatDAO {
    private static final String LOCK_TABLE_CHATBOX_BY_ID = "SELECT * FROM tblChatBox WHERE id = ? FOR UPDATE";
    private static final String LOCK_TABLE_CHATBOX = "SELECT * FROM tblChatBox FOR UPDATE";
    private static final String LOCK_TABLE_CHATCONTENT_BY_ID = "SELECT * FROM tblChatBoxContent WHERE id = ? FOR UPDATE";
    private static final String LOCK_TABLE_CHATCONTENT = "SELECT * FROM tblChatBoxContent FOR UPDATE";
    ...
    public static class BoxManager {
        ...
        public static synchronized java.util.List<model.ChatBox> getBoxes(Connection connection, int userId) throws SQLException {
            try (Statement lockStatement = connection.createStatement();
                ResultSet lockResultSet = lockStatement.executeQuery(LOCK_TABLE_CHATBOX)) {
        ...
        public static synchronized model.ChatBox getBox(Connection connection, int user1, int user2) throws SQLException {
            try (Statement lockStatement = connection.createStatement();
                ResultSet lockResultSet = lockStatement.executeQuery(LOCK_TABLE_CHATBOX)) {
    ...
    public static class ContentManager {
        ...
        "    ?, ?, ?, ?\n" + //
        ");";
        public static synchronized void createContent(Connection connection, int box, int sender, String message) throws SQLException {
            try (Statement lockStatement = connection.createStatement();
                ResultSet lockResultSet = lockStatement.executeQuery(LOCK_TABLE_CHATCONTENT)) {
        ...
        public static synchronized java.util.List<model.ChatContent> getContent(Connection connection, int box) throws SQLException {
            try (Statement lockStatement = connection.createStatement();
                ResultSet lockResultSet = lockStatement.executeQuery(LOCK_TABLE_CHATCONTENT)) {
