...
public class MailService {
    public static void sendMail(int userID, java.util.List<model.OrderedItem> orderedItems)
        ...
        body.append(", </h2>");
        body.append("<h2>You have purchased the following items: </h2>");

        body.append("<table border='1'><tr><th>Name</th><th>Quantity</th><th>Price</th></tr>");

        for (final model.OrderedItem cartItem : orderedItems) {

            // body += "\n\t" + cartItem.getProduct().getName();
            body.append("<tr>")
                    .append("<td>").append(cartItem.getProductItemId().getProductId().getName()).append("</td>")
                    .append("<td>").append(cartItem.getQuantity()).append("</td>")
                    .append("<td>").append("$").append(cartItem.getProductItemId().getPrice()).append("</td>")
                    .append("</tr>");
        }

        body.append("</table></body></html>");

        message.setSubject("[TEST] Thank you for your purchase!");
